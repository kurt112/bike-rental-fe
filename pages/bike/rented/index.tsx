...
const Rented: NextPage = ({bikes, settings}: any) => {
    ...
    const searchClick = async () => {
        await router.push(`/bike/rented?search=${pagination.search}&page=${1}&size=${size}&status=${getBikeStatus.RENTED}`)
    }
                ...
                <ul className="inline-flex items-center -space-x-px mr-5">
                    {
                        page > 1 ? <li className='cursor-pointer'>
                            <Link
                                href={`/bike/rented?search=${search}&page=${parseInt(page) - 1}&size=${size}&status=${getBikeStatus.RENTED}`}>
                                <button
                        ...
                        Array.from(Array(settings.totalPages).keys()).map((page) => {
                            return (
                                <li key={page} className={'cursor-pointer'}>
                                    <Link
                                        href={`/bike/rented?search=${search}&page=${page + 1}&size=${size}&status=${getBikeStatus.RENTED}`}>
                                        <div
                    ...
                    }
                    {
                        page >= settings.totalPages ? null :
                            <Link
                                href={`/bike/rebnted?search=${search}&page=${parseInt(page) + 1}&size=${size}&status=${getBikeStatus.RENTED}`}>
